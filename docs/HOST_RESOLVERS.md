...
2. It walks the list in order and, for each mirror whose host has a matching *already-registered* resolver, calls that resolver's `ResolveAsync` — reusing its full HTTP/WebView2 behavior unchanged. The original MultiUp link's filename/package/part metadata is preserved on the delegated `LinkCandidate` (mirror URLs rarely embed a friendly filename).
3. **If a matched mirror's `ResolveAsync` throws (daily quota hit, file removed from that host, temporary outage, unsolved captcha, etc.), it does not give up** — it records the error and tries the next matching mirror in the list, since MultiUp lists the same file on multiple hosts for exactly this reason. It only fails once every matched mirror has been tried and failed, and the resulting error lists what was tried and why each attempt failed.
4. If none of the listed mirrors match a supported host at all, it throws with the exact list of (unsupported) hosts offered, so the failure is actionable instead of a generic error.
