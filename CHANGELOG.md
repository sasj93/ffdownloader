...
- MultiUp.io (and legacy MultiUp.org) host support: a meta-resolver that fetches the site's mirror list and tries each already-supported mirror in turn, preserving the original filename/package metadata. If a mirror fails (daily quota hit, file removed, temporary outage, etc.) it automatically falls back to the next matching mirror instead of giving up, only failing once every matched mirror has been tried; fails with the exact list of unsupported mirror hosts when none match at all.
